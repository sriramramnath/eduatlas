# Chapter 23: Radiation and Half-Life
## Pearson Edexcel IGCSE Physics - Concise Synopsis

---

## Background Radiation

### Main Sources
| Source | ~% |
|--------|-----|
| Radon gas | 50% |
| Rocks/soil | 14% |
| Food/drink | 12% |
| Cosmic rays | 10% |
| Medical | 15% |

---

## Detecting Radiation

### GM Tube
- Radiation ionizes gas
- Creates electrical pulse
- Counter records activity

### Film Badge
- Worn by radiation workers
- Measures cumulative dose
- Different filters identify types

---

## Half-Life

### Definition
Time for:
- Half the nuclei to decay, OR
- Activity to halve

### Formula
```
Remaining = Initial × (½)ⁿ
n = time ÷ half-life
```

### Example Half-Lives
| Isotope | Half-Life |
|---------|-----------|
| U-238 | 4.5 billion yrs |
| C-14 | 5730 yrs |
| Co-60 | 5.3 yrs |
| I-131 | 8 days |
| Tc-99m | 6 hrs |

---

## Decay Curve

```tex
\begin{tikzpicture}[x=1cm,y=0.9cm,>=stealth,font=\small]
  \draw[->] (0,0) -- (5.8,0) node[right] {Time};
  \draw[->] (0,0) -- (0,4.4) node[above] {Activity};
  \draw[smooth,thick] plot coordinates {(0,4) (1,3.15) (2,2.45) (3,1.85) (4,1.35) (5,0.95)};
\end{tikzpicture}
```
- Exponential decay
- Constant half-life

---

## Activity

- **Unit:** Becquerel (Bq)
- **1 Bq** = 1 decay per second
- Activity ∝ number of undecayed nuclei

---

## Random Nature

- **Random** - Cannot predict which nucleus
- **Spontaneous** - Cannot trigger decay
- **Constant probability** - Per unit time

---

## Key Equations

```
Half-life: t½
Number of half-lives: n = t / t½
Remaining: N = N₀ × (½)ⁿ
```

---

## Quick Checklist

- [ ] Know background radiation sources
- [ ] Understand half-life definition
- [ ] Can calculate remaining activity/mass
- [ ] Can interpret decay curves
- [ ] Know units (Bq)
- [ ] Understand random nature of decay

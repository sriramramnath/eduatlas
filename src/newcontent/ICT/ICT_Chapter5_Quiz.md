# IGCSE ICT — Chapter 5: Networks
## Quiz Section (Pearson Edexcel Specification)

---

## SECTION A: Multiple Choice Questions

### Question 1
Which network type covers the largest geographical area?

A) LAN  
B) WAN  
C) PAN  
D) WLAN  

**Correct Answer: B) WAN**

**Explanation:** WAN (Wide Area Network) spans large geographical areas such as cities, countries, or continents. The internet is the largest WAN. LAN covers a single building, PAN covers just a few metres, and WLAN is a wireless LAN.

---

### Question 2
Which device connects different networks together?

A) Switch  
B) Hub  
C) Router  
D) Modem  

**Correct Answer: C) Router**

**Explanation:** Routers connect different networks together, such as connecting a LAN to the internet (WAN). Switches connect devices on the same network. Hubs are basic devices that broadcast to all ports. Modems connect to ISP networks.

---

### Question 3
In which network topology do all devices connect to a central device?

A) Bus topology  
B) Ring topology  
C) Star topology  
D) Mesh topology  

**Correct Answer: C) Star topology**

**Explanation:** In star topology, all devices connect to a central device (usually a switch or hub). This is the most common LAN topology because it's easy to troubleshoot and a single device failure doesn't affect others.

---

### Question 4
What does DHCP do?

A) Encrypts network traffic  
B) Assigns IP addresses automatically  
C) Connects wireless devices  
D) Filters network traffic  

**Correct Answer: B) Assigns IP addresses automatically**

**Explanation:** DHCP (Dynamic Host Configuration Protocol) automatically assigns IP addresses and network configuration to devices when they connect to a network. This eliminates manual configuration and prevents IP address conflicts.

---

### Question 5
Which address is permanently assigned to a network interface card by the manufacturer?

A) IP address  
B) MAC address  
C) DNS address  
D) Web address  

**Correct Answer: B) MAC address**

**Explanation:** MAC (Media Access Control) addresses are permanently assigned to network hardware by manufacturers. IP addresses can change and are assigned by DHCP or manually. MAC addresses are used for local network communication.

---

### Question 6
What is the main disadvantage of a star topology?

A) Difficult to add new devices  
B) If the central device fails, the entire network fails  
C) Uses too much cable  
D) Slow data transfer speeds  

**Correct Answer: B) If the central device fails, the entire network fails**

**Explanation:** In star topology, all devices connect to a central device. If this central device (switch or hub) fails, all connected devices lose network connectivity. This is the main weakness of star topology.

---

### Question 7
Which device forwards data only to the intended recipient port?

A) Hub  
B) Switch  
C) Modem  
D) Repeater  

**Correct Answer: B) Switch**

**Explanation:** Switches learn the MAC addresses of connected devices and forward data only to the port where the recipient device is connected. Hubs broadcast data to all ports regardless of destination.

---

### Question 8
What does VPN stand for?

A) Virtual Private Network  
B) Virtual Public Network  
C) Variable Private Network  
D) Verified Private Network  

**Correct Answer: A) Virtual Private Network**

**Explanation:** VPN (Virtual Private Network) creates a secure, encrypted connection over a public network like the internet. It allows remote users to securely access private networks.

---

### Question 9
Which security device filters incoming and outgoing network traffic?

A) Router  
B) Switch  
C) Firewall  
D) Modem  

**Correct Answer: C) Firewall**

**Explanation:** A firewall monitors and controls incoming and outgoing network traffic based on predetermined security rules. It acts as a barrier between trusted and untrusted networks.

---

### Question 10
What is the purpose of a Wireless Access Point (WAP)?

A) Connect to the internet  
B) Allow wireless devices to connect to a wired network  
C) Assign IP addresses  
D) Block unauthorised access  

**Correct Answer: B) Allow wireless devices to connect to a wired network**

**Explanation:** A WAP (Wireless Access Point) enables wireless devices to connect to a wired network using Wi-Fi. It bridges wireless and wired network segments.

---

### Question 11
Which IPv4 address format is correct?

A) 192.168.1.256  
B) 192.168.1.1  
C) 192.168.1.1.1  
D) 192.168.1  

**Correct Answer: B) 192.168.1.1**

**Explanation:** IPv4 addresses consist of four numbers separated by dots, each ranging from 0 to 255. 192.168.1.1 is a valid private IP address format. 256 is invalid (too high), and the other options have wrong numbers of segments.

---

### Question 12
What happens in a mesh topology if one connection fails?

A) The entire network fails  
B) Data can still reach its destination via alternative paths  
C) Only that device disconnects  
D) The network becomes slower but works  

**Correct Answer: B) Data can still reach its destination via alternative paths**

**Explanation:** In mesh topology, devices have multiple connections to other devices. If one connection fails, data can be rerouted through alternative paths, making mesh networks highly fault-tolerant.

---

---

## SECTION B: True/False Questions

### Statement 1
**A switch is more intelligent than a hub because it sends data only to the intended recipient.**

**Answer: TRUE**

**Explanation:** Switches learn MAC addresses and create a table mapping devices to ports. They forward data only to the correct port, unlike hubs which broadcast to all ports. This makes switches more efficient and secure.

---

### Statement 2
**LAN covers a larger geographical area than WAN.**

**Answer: FALSE**

**Correction:** WAN (Wide Area Network) covers a larger geographical area than LAN. LAN (Local Area Network) is limited to a building or campus, while WAN can span cities, countries, or globally.

---

### Statement 3
**IP addresses can change, but MAC addresses are permanent.**

**Answer: TRUE**

**Explanation:** IP addresses are assigned by DHCP or manually and can change when a device reconnects to a network. MAC addresses are permanently assigned to network hardware by the manufacturer and cannot be changed.

---

### Statement 4
**In a bus topology, all devices connect to a central switch.**

**Answer: FALSE**

**Correction:** In a bus topology, all devices connect to a single central cable (the bus). Star topology uses a central switch or hub. Bus topology is rarely used in modern networks.

---

### Statement 5
**A router can connect a LAN to the internet.**

**Answer: TRUE**

**Explanation:** Routers connect different networks together, including connecting a LAN to the internet (WAN). They route packets between networks and often provide NAT (Network Address Translation) for internet access.

---

### Statement 6
**WPA3 is more secure than WPA2 for Wi-Fi networks.**

**Answer: TRUE**

**Explanation:** WPA3 is the latest Wi-Fi security standard, offering improved encryption and security features over WPA2. WPA3 provides stronger encryption and better protection against password-guessing attacks.

---

### Statement 7
**A hub forwards data to all connected ports.**

**Answer: TRUE**

**Explanation:** Hubs are basic networking devices that broadcast incoming data to all connected ports regardless of the intended destination. This is inefficient compared to switches and is why hubs are largely obsolete.

---

### Statement 8
**Static IP addresses are assigned automatically by DHCP.**

**Answer: FALSE**

**Correction:** Static IP addresses are manually configured and do not change. DHCP automatically assigns dynamic IP addresses. Static IPs are used for servers, printers, and network devices that need consistent addresses.

---

### Statement 9
**The internet is an example of a WAN.**

**Answer: TRUE**

**Explanation:** The internet is the largest example of a Wide Area Network (WAN), connecting millions of networks globally across countries and continents. It spans the entire world.

---

### Statement 10
**A proxy server hides the client's IP address.**

**Answer: TRUE**

**Explanation:** A proxy server acts as an intermediary between clients and servers. When using a proxy, requests go through the proxy server, so the destination server sees the proxy's IP address, not the client's.

---

### Statement 11
**Fibre optic cables are commonly used within LANs for connecting end-user devices.**

**Answer: FALSE**

**Correction:** Fibre optic cables are typically used for WAN connections and backbone links due to their high cost. LANs usually use copper Ethernet cables (Cat5e, Cat6) for connecting end-user devices, as they are more cost-effective for short distances.

---

### Statement 12
**A modem converts digital signals to analogue signals for transmission over telephone lines.**

**Answer: TRUE**

**Explanation:** A modem (modulator-demodulator) converts digital signals from a computer to analogue signals for transmission over telephone or cable lines, and converts incoming analogue signals back to digital.

---

---

## SECTION C: Short Answer Questions

### Question 1 (1 mark)
**State one advantage of using a star topology for a network.**

**Mark Scheme Answer:**
Any one of the following:
- Easy to add new devices [1 mark]
- One device failure doesn't affect others [1 mark]
- Easy to troubleshoot problems [1 mark]
- Better network performance [1 mark]
- Centralised management [1 mark]

---

### Question 2 (2 marks)
**Explain the difference between a switch and a hub.**

**Mark Scheme Answer:**
- A switch sends data only to the intended recipient port [1 mark]
- A hub broadcasts data to all connected ports [1 mark]
- Switches are more efficient and reduce unnecessary traffic [1 mark]
- Switches can operate at full-duplex; hubs are half-duplex only [1 mark]

*Award up to 2 marks for clear distinction between the two devices.*

---

### Question 3 (2 marks)
**Describe two advantages of using DHCP on a network.**

**Mark Scheme Answer:**
Any two advantages:
- Eliminates manual IP address configuration [1 mark]
- Prevents IP address conflicts [1 mark]
- Efficient use of available IP addresses [1 mark]
- Simplifies network management [1 mark]
- Easier to add new devices to network [1 mark]

---

### Question 4 (3 marks)
**Compare LAN and WAN networks. Include scope, ownership, and speed.**

**Mark Scheme Answer:**
**Scope:**
- LAN covers a limited area (building/campus) [1 mark]
- WAN covers large areas (cities/countries/global) [1 mark]

**Ownership:**
- LAN usually owned by single organisation [1 mark]
- WAN typically involves third-party providers [1 mark]

**Speed:**
- LAN typically faster (100Mbps-10Gbps) [1 mark]
- WAN speeds vary and are usually slower than LANs [1 mark]

*Award up to 3 marks for comparison covering all aspects.*

---

### Question 5 (3 marks)
**Explain three functions of a router.**

**Mark Scheme Answer:**
Any three functions:
- Connects different networks together [1 mark]
- Routes data packets to correct destinations [1 mark]
- Provides DHCP to assign IP addresses [1 mark]
- Performs NAT for internet access [1 mark]
- Provides firewall/security features [1 mark]
- Manages traffic with QoS [1 mark]

---

### Question 6 (4 marks)
**A school wants to set up a network in a computer room. Discuss the advantages and disadvantages of using a star topology.**

**Mark Scheme Answer:**
**Advantages:**
- Easy to add new computers as needed [1 mark]
- If one computer fails, others continue working [1 mark]
- Easy to identify and fix problems [1 mark]
- Good performance for classroom use [1 mark]
- Central switch in accessible location [1 mark]

**Disadvantages:**
- If switch fails, all computers lose connection [1 mark]
- Requires more cable than bus topology [1 mark]
- Cost of central switch [1 mark]
- Switch could be bottleneck if overloaded [1 mark]

**Recommendation:** Star topology is recommended because the benefits outweigh the risks, and a backup switch can be kept for emergencies. [1 mark]

*Award up to 4 marks for balanced discussion.*

---

### Question 7 (4 marks)
**Explain the difference between MAC addresses and IP addresses, including their purposes.**

**Mark Scheme Answer:**
**MAC Address:**
- Unique hardware address assigned by manufacturer [1 mark]
- Permanent, cannot be changed [1 mark]
- Used for local network communication [1 mark]
- Format: hexadecimal (e.g., 00:1A:2B:3C:4D:5E) [1 mark]

**IP Address:**
- Network address for routing data [1 mark]
- Can change (assigned by DHCP or manually) [1 mark]
- Used for network-to-network communication [1 mark]
- Format: IPv4 (192.168.1.1) or IPv6 [1 mark]

*Award up to 4 marks for comprehensive comparison.*

---

### Question 8 (4 marks)
**Describe four security measures that can be used to protect a network.**

**Mark Scheme Answer:**
Any four measures:

1. **Firewall** — Filters incoming/outgoing traffic, blocks unauthorised access [1 mark]

2. **VPN** — Creates encrypted connection for remote access, secures data transmission [1 mark]

3. **Strong passwords** — Prevents unauthorised access to network resources [1 mark]

4. **WPA2/WPA3 encryption** — Secures wireless network from eavesdropping [1 mark]

5. **Regular updates** — Patches security vulnerabilities in software [1 mark]

6. **Access control** — Limits network access to authorised users only [1 mark]

7. **Proxy server** — Filters content and hides internal IP addresses [1 mark]

*Award up to 4 marks for four distinct security measures with explanations.*

---

## Mark Summary

| Section | Questions | Marks Available |
|---------|-----------|-----------------|
| MCQs | 12 | 12 |
| True/False | 12 | 12 |
| Short Answer | 8 | 23 |
| **Total** | **32** | **47** |

---

*These quiz questions are designed to test understanding of the Pearson Edexcel IGCSE ICT specification for Chapter 5: Networks. Questions reflect exam-style format and mark scheme requirements.*
